...
namespace SliBoxEngine
{
    public class SliBoxBehaviour : MonoBehaviour
    {
        ...
        {
            public static void SetRectTransformMaxSize(RectTransform rectTransform, Vector2 nativeSize, float maxSize)
            {
                float currentDeltaSize = nativeSize.x > nativeSize.y ? nativeSize.x : nativeSize.y;

                rectTransform.sizeDelta = nativeSize * maxSize / (float)currentDeltaSize;
            }
        }
